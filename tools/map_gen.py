...
class TennisCourtMapGenerator:
    ...
    def add_snow_patches(self, court_info, num_patches=8):
        ...
        print(f"   코트 범위: Row[{start_row}-{end_row}], Col[{start_col}-{end_col}]")
        print(f"   네트 위치: Row {net_row}")
    ...
    def preview_map_ascii(self, court_info):
        """간단한 ASCII 프리뷰"""
        print("\n📍 맵 프리뷰 (간략화):")
        ...
        # 20x20 샘플링
        for i in range(0, GRID_HEIGHT, GRID_HEIGHT // 20):
            line = ""
            for j in range(0, GRID_WIDTH, GRID_WIDTH // 40):
...
def generate_tennis_court_map(with_snow=True, num_snow_patches=8):
    ...
    # ASCII 프리뷰
    generator.preview_map_ascii(court_info)
    
    # 저장
    filename = 'maps/TennisCourt_Clustered.pkl' if with_snow else 'maps/TennisCourt_Clean.pkl'
